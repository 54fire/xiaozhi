...
void Protocol::SendSensorDetected(const std::string &sensor, const int &sensorValue, const bool wakup)
{

    std::string json = "{\"session_id\":\"" + session_id_ +
                       "\",\"type\":\"listen\",\"wakeup\":\"" + (wakup ? "true" : "false") + "\",\"sensor\":\"" + sensor + "\",\"sensor_value\":\"" + std::to_string(sensorValue) + "\"}";
    SendText(json);
}
