...
void Application::CloseProtocol() {
...
void Application::Start()
{
    ...
    if (board.IsConnected()) {
        ...
        ESP_LOGE(TAG, "Device-Id: %s Client-Id: %s", SystemInfo::GetMacAddress().c_str(), board.GetUuid().c_str());
    ...
    while (true)
    {
        ...
        if (Sensor::Consume_Queue(&msg))
        {
            ...
            case HEAD2:
                ...
                break;
            case LEFT_HAND:
                sensor_msg_ = "left_hand";
                break;
            case RIGHT_HAND:
                sensor_msg_ = "right_hand";
                break;
            }

            ESP_LOGI(TAG, "Event:%s , Value: %d", sensor_msg_.c_str(), msg.value);
            if (device_state_ == kDeviceStateIdle)
            {
                SetDeviceState(kDeviceStateConnecting);
                if (!protocol_->OpenAudioChannel())
                {
                    ESP_LOGE(TAG, "sensor:Failed to open audio channel");
                    SetDeviceState(kDeviceStateIdle);
                    continue;
                }
