...
class CqgcC3BluFiBoard : public BlufiBoard
{
private:
...
public:
    CqgcC3BluFiBoard() : boot_button_(BOOT_BUTTON_GPIO), next_button_(NEXT_BUTTON_GPIO), prev_button_(PREV_BUTTON_GPIO)
    {
...
DECLARE_BOARD(CqgcC3BluFiBoard);
    ...
    public:
        PressToTalk() : Thing("PressToTalk", "控制对话模式，一种是长按对话，一种是单击后连续对话。")
        {
            ...
            properties_.AddBooleanProperty("enabled", "true 表示长按说话模式，false 表示单击说话模式", []() -> bool {
                auto board = static_cast<CqgcC3BluFiBoard*>(&Board::GetInstance());
