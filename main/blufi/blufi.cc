...
static void example_event_callback(esp_blufi_cb_event_t event, esp_blufi_cb_param_t *param)
{
	...
	case ESP_BLUFI_EVENT_RECV_SOFTAP_PASSWD:
        ...
        esp_wifi_set_config(WIFI_IF_AP, &ap_config);
        BLUFI_INFO("Recv SOFTAP PASSWORD %s len = %d", ap_config.ap.password, param->softap_passwd.passwd_len);
        break;
...
Blufi::Blufi()
{
    ...
    if (ret) {
        BLUFI_ERROR("%s BLUFI controller init failed: %s", __func__, esp_err_to_name(ret));
        return;
    }
#endif
