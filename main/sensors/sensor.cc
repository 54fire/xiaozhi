...
adc_oneshot_unit_handle_t Sensor::adc1_handle = nullptr;
adc_cali_handle_t Sensor::adc1_cali_chan0_handle = nullptr;
...
void Sensor::PressureSensor_Task(void *pvParameters)
{
    ...
    while (true)
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            if (pressurePins[i] != -1)
            {
                int adc_value = MAX_VALUE - read_adc();
...
void Sensor::init_adc()
{
    ...
    ESP_ERROR_CHECK(adc_oneshot_config_channel(adc1_handle, HUG1_CHANNEL, &config)); // 假设IO8对应的ADC通道是7

    // 校准ADC
    adc_cali_handle_t adc1_cali_chan0_handle = NULL;
    bool do_calibration = example_adc_calibration_init(ADC_UNIT_1, HUG1_CHANNEL, ADC_ATTEN_DB_12, &adc1_cali_chan0_handle);

    // 存储ADC句柄和校准句柄
    Sensor::adc1_handle = adc1_handle;
    Sensor::adc1_cali_chan0_handle = do_calibration ? adc1_cali_chan0_handle : NULL;
}

int Sensor::read_adc()
{
    if (Sensor::adc1_handle && Sensor::adc1_cali_chan0_handle)
    {
        int adc_raw_value;
        ESP_ERROR_CHECK(adc_oneshot_read(Sensor::adc1_handle, HUG1_CHANNEL, &adc_raw_value));
        // ESP_LOGI(TAG, "ADC1 Channel[7] Raw Data: %d", adc_raw_value);

        int voltage_value;
        if (Sensor::adc1_cali_chan0_handle)
        {
            ESP_ERROR_CHECK(adc_cali_raw_to_voltage(Sensor::adc1_cali_chan0_handle, adc_raw_value, &voltage_value));
